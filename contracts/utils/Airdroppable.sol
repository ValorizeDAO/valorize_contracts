...
abstract contract Airdroppable {
    ...
    mapping (uint => Airdrop) airdrops;
		 ...
		 */
		function _newAirdrop(bytes32 _merkleRoot, uint256 _timeLimit) internal returns (uint256 airdropId) {
		 ...
		 */
		function _sweepTokens(address _destination, uint _internalBalanceToSweep) internal {
				require(numberOfAirdrops > 0, "No airdrops active");
