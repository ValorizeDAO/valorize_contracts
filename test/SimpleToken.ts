...
const TEN_MILLION_TOKENS = ethers.BigNumber.from(
...
describe("SimpleToken", () => {
  ...
  describe("Deployment", () => {
    ...
    it("should supply the vault the initial free supply amount", async () => {
      expect(await simpleToken.balanceOf(await vault.getAddress())).to.equal(
        FREE_SUPPLY
      );
    });
    ...
    it("should hold the airdropped tokens within the contract", async () => {
      expect(
        await simpleToken.balanceOf(await simpleToken.resolvedAddress)
      ).to.equal(AIRDROP_SUPPLY);
    });

    it("should record what was the initial supply", async () => {
      expect(await simpleToken.getInitialSupply()).to.equal(
        FREE_SUPPLY.add(AIRDROP_SUPPLY)
      );
    });
  });
  ...
  describe("Airdrop", async () => {
    ...
    const setMerkleRoot = async () => {
      ...
      await simpleToken
        .connect(admin1)
        .newAirdrop(root, BigNumber.from("100000000000"));
    };
    ...
    it("should emit a MerkleRootChanged Event on setting", async () => {
      ...
      const timeDelay = BigNumber.from("10000000000");
      const expectedAirdropEnd = BigNumber.from(blockTimestamp)
        .add(timeDelay)
        .add(1);
      await ethers.provider.send("evm_mine", [parseInt(blockTimestamp, 10)]);
      await expect(simpleToken.connect(admin1).newAirdrop(root, timeDelay))
    ...
    it("should allow people to claim their alloted tokens", async () => {
      ...
      expect(
        await simpleToken.balanceOf(await addresses[0].getAddress())
  ...
  describe("Sweep", async () => {
    ...
    it("should set isComplete to false on creation", async () => {
      const { isComplete } = await simpleToken
        .connect(addresses[0])
        .getAirdropInfo(0);
    ...
    it("should not allow you to create a new airdrop if the previous one is not finished", async () => {
      await expect(
        simpleToken
          .connect(admin1)
          .newAirdrop(merkleTree.getHexRoot(), BigNumber.from("100000000000"))
    ...
    it("should only allow you to finish an airdrop if the claimperiod has ended", async () => {
      ...
      await expect(simpleToken.connect(admin1).completeAirdrop()).to.be.ok;
    });

    it("should emit an AirdropComplete event when completing an airdrop", async () => {
      await ethers.provider.send("evm_increaseTime", [100000000010]);
      await ethers.provider.send("evm_mine", []);
      await expect(simpleToken.connect(admin1).completeAirdrop())
    ...
    it("should create an airdrop with a new index each time 'newAirdrop' is called", async () => {
      const root = merkleTree.getHexRoot();
      expect(await simpleToken.numberOfAirdrops()).to.equal(
        BigNumber.from("1")
      );
      ...
      await expect(await simpleToken.numberOfAirdrops()).to.equal(
        BigNumber.from("3")
      );
    });
    ...
    it("should allow you to sweep the leftover funds if no airdrop is running", async () => {
      ...
      expect(await simpleToken.balanceOf(await admin1.getAddress())).to.equal(
        BigNumber.from(0)
      );
