...
chai.use(solidity);
const { expect } = chai;
const INITIAL_SUPPLY_AMOUNT = ethers.BigNumber.from(
...
describe("CreatorToken", () => {
  ...
  const setupCreatorToken = async () => {
    ...
    token = await CreatorToken.deploy(
  ...
  describe("Deployment", () => {
    ...
    it("Should let user buy tokens from the contract, and the price increasing the more ether is staked", async () => {
      ...
      const twoEthTxMetadata = {
        value: ONE_ETH.mul(ethers.BigNumber.from("2")),
      };
      await token.connect(addr1).buyNewTokens(oneEthTxMetadata);
      let senderBalance = (await token.balanceOf(
        await addr1.getAddress()
      )) as BigNumber;
  ...
  describe("Founder Percentage", () => {
    ...
    it("Should not give all newly minted tokens to buyer if percentage is 100", async () => {
      const oneFinneyTxMetadata = {
        value: ethers.utils.parseUnits("1.0", "finney").toNumber(),
      };
    ...
    it("Should not give any newly minted tokens to founder if percentage is 0", async () => {
      ...
      await token.connect(addr1).buyNewTokens(oneFinneyTxMetadata);
      const ownerTokenBalanceAfterTransaction = await token.balanceOf(
        await owner.getAddress()
      );
      const senderBalance = await token.balanceOf(await addr1.getAddress());
      expect(senderBalance).to.equal(
        ethers.BigNumber.from("93204408653311889783")
      );
  ...
  describe("Withdrawals", () => {
    ...
    it("Should allow users to withdraw all the ETH from the contract", async () => {
      ...
      const ownerSupply = await token
        .connect(owner)
        .balanceOf(await owner.getAddress());

      await token.connect(owner).sellTokensForEth(ownerSupply);
      const finalOwnerBalance = parseFloat(
        ethers.utils.formatEther(await owner.getBalance())
      );
      expect(
        floatDifferenceIsWithinDelta(initialOwnerBalance, finalOwnerBalance)
      ).to.equal(true);
      expect(
        await ethers.getDefaultProvider().getBalance(token.address)
      ).to.equal(0);
    });
    ...
    it("The ETH withdrawal should be proportionate to the balance in the contract", async () => {
      ...
      const userTokenBalance = (await token
        .connect(addr1)
        .balanceOf(await addr1.getAddress())) as BigNumber;
    ...
    it("Should not allow users to withdraw more tokens than what they own", async () => {
      await token.connect(addr1).buyNewTokens(oneFinneyTxMetadata);
      const userSupply = await token
        .connect(addr1)
        .balanceOf(await addr1.getAddress());

      await expect(
        token
          .connect(addr1)
          .sellTokensForEth(userSupply.add(ethers.BigNumber.from("1")))
  ...
  describe("View", () => {
    ...
    it("Should let users see how many tokens will be deployed", async () => {
      const [toBuyer, toOwner] = await token
        .connect(addr1)
        .calculateTokenBuyReturns(ONE_FINNEY);
      expect(toBuyer.toString()).to.equal("83883967787980700804");
      expect(toOwner.toString()).to.equal("9320440865331188978");
      const [toBuyer2, toOwner2] = await token
        .connect(addr1)
        .calculateTokenBuyReturns(ethers.utils.parseUnits("2.0", "ether"));
      expect(toBuyer2.toString()).to.equal("54023820594276909688566");
      await token.connect(addr1).buyNewTokens({ value: ONE_FINNEY });
      expect(
        await token.connect(addr1).balanceOf(await addr1.getAddress())
      ).to.equal(toBuyer);
    });
    ...
    it("Should let users see how many tokens will be deployed", async () => {
      await token
        .connect(addr1)
        .buyNewTokens({ value: ethers.utils.parseUnits("1.0", "ether") });
    ...
    it("Should let users see how much ETH will be returned from selling tokens", async () => {
      ...
      const userTokenBalance = (await token
        .connect(owner)
        .balanceOf(await owner.getAddress())) as BigNumber;

      const saleReturns = await token
        .connect(owner)
        .calculateTotalSaleReturn(userTokenBalance);
      ...
      expect(expectedBalance.sub(finalUserEthBalance).lt(maxGasFee)).to.equal(
        true
      );
    });
  });
...
const floatDifferenceIsWithinDelta = (
  floatOne: number,
  floatTwo: number,
  delta = 0.001
): Boolean => {
